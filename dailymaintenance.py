...
Args:
    ...
    vmkey (-k) - key in keyvalue pair of tag to match against
    vmvtag (-t) - value in keyvalue pair of tag to match against

    Stops the virtual machines
    Example 1:
            stop_azure_vm.py -g <resourcegroupname> -k <key for tag> -t <value for tag>
            stop_azure_vm.py -g <resourcegroupname> -v <vmname>
            stop_azure_vm.py -g <resourcegroupname>
            stop_azure_vm.py

Changelog:
    2019-06-20 albrokke@microsoft.com - added filtering by tag
...
opts, args = getopt.getopt(sys.argv[1:], "g:v:k:t")
for o, a in opts:
    if o == '-g':  # if resource group name is passed with -g option then take it
        resource_group_name = a
    if o == '-k': #if tag key is passed with -k option then take it
        tag_key = a
    if o == '-t' : #if tag value is passed with -k option then take it
        tag_value = a
...
if tag_key is not None and tag_value is None:
    raise ValueError("tag key argument was passed in without a matching tag value -t specified")
if tag_value is not None and tag_key is None:
    raise ValueError("tag value argument was passed in without a matching key -k value specified")
...
for group in groups:
    ...
    for vm in vms:
        ...
        if tag_key is not None:
            vm_tags = vm_detail.tags
            print(vm_tags)
            if vm_tags == "" or vm_tags == None:
                continue
            else:
                print(tag_key)
                print(vm_tags.get(tag_key))
            if vm_tags.get(tag_key) == tag_value:
                    ""
            else:
                continue

        if vm_detail.instance_view.statuses[1].code == 'PowerState/running' and safeshutdown == 'true' :
